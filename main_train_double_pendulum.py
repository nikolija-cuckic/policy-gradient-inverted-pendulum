...
def train_double_pendulum(seed=1, total_episodes=5000):
    env_name = "InvertedDoublePendulum-v5"  # <--- JEDINA PROMENA
    
    print(f"--- Treniram Dvostruko Klatno (Seed {seed}) ---")
    env = gym.make(env_name)
    set_seed(seed)
    
    # Automatska detekcija dimenzija (6 za double pendulum)
    ...
    print(f"✓ Trening završen! Rezultati u logs/double_pendulum_seed{seed}.csv")

if __name__ == "__main__":
    # Treniramo samo jedan seed da uštedimo vreme (ali možeš listu [1, 2, 3])
    train_double_pendulum(seed=1)
